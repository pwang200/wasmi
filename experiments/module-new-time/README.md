...
Same host for both. Create cases live in `create_cases/`; Finish cases live in `finish_cases/`; step-1 total kernels (no 100 KB pad) live in `total_cases/`; real totals (Create-7 combo dummies to 100 KB + a Finish kernel) live in `real_total/`.
...
# real total (Create-7 filler + Finish kernel)
cargo run -p module-new-time --release --bin gen-real-total-a
cargo run -p module-new-time --release --bin module-new-time -- \
    experiments/module-new-time/real_total/t-a-30k-nest-unroll/case.wasm
```
